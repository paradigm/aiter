...
def _load_modules_from_config():
    """Load module names from the JIT compiler configuration."""
...
def get_exclude_ops():
    """Determine which ops to exclude based on PREBUILD_KERNELS level."""
...
def _prepare_aiter_meta():
    """Prepare aiter_meta directories for packaging.

    This must run before setup() because aiter_meta is declared in packages=[].
    This is a fast operation (just file copies), not the expensive build.
    """
...
def _cleanup_aiter_meta():
    """Clean up aiter_meta directory after setup completes."""
    if os.path.exists("aiter_meta") and os.path.isdir("aiter_meta"):
        shutil.rmtree("aiter_meta")


# Prepare aiter_meta before setup() - required for metadata generation
_prepare_aiter_meta()

# Register cleanup to run after setup completes
import atexit
atexit.register(_cleanup_aiter_meta)


class AiterBuildExt(BuildExtension):
    """
    Custom build extension that defers ROCm validation and kernel
    precompilation to the actual build phase.

    This fixes the setuptools anti-pattern where building happened during
    setup() import instead of when actually needed. Saves significant time
    on operations like 'pip download' that don't require building.
    """
    ...
    def run(self):
        """Run the build process only when actually building extensions."""
        self._validate_rocm_environment()
        self._run_prebuild_kernels()

        # Call parent run() to handle standard extension building
        super().run()

        # Copy built .so files to package for wheel inclusion
        self._copy_built_modules_to_package()

    def _validate_rocm_environment(self):
        """Validate that ROCm environment is properly set up."""
        if not IS_ROCM:
    ...
    def _run_prebuild_kernels(self):
        """Run prebuild kernels logic if PREBUILD_KERNELS is set."""
        ...
        # Handle MHA variants for PREBUILD_KERNELS == 1
        if PREBUILD_KERNELS == 1:
            extra_args_build = []
            req_md_names = [
        ...
        # Build modules using ThreadPoolExecutor
    ...
    def _copy_built_modules_to_package(self):
        """Copy built .so files to build_lib for inclusion in wheel."""
        if PREBUILD_KERNELS == 0:
            return

        # Get the jit directory where .so files are built
        jit_dir = core.get_user_jit_dir()

        # Target directory in build_lib
        if hasattr(self, 'build_lib') and self.build_lib:
            target_dir = os.path.join(self.build_lib, "aiter", "jit")
            os.makedirs(target_dir, exist_ok=True)

            # Copy all .so files
            so_files = glob.glob(f"{jit_dir}/*.so")
            if so_files:
                for so_file in so_files:
                    dest = os.path.join(target_dir, os.path.basename(so_file))
                    shutil.copy2(so_file, dest)
                print(f"[aiter] Copied {len(so_files)} .so files to {target_dir}")
            else:
                print(f"[aiter] Warning: No .so files found in {jit_dir}")
...
setup(
    ...
    cmdclass={"build_ext": AiterBuildExt},
    python_requires=">=3.8",
    install_requires=[
        "pybind11>=3.0.1",
        "ninja",
